...
export const card = (() => {
  ...
  const renderBody = async (c) => {
    ...
    if (c.gif_url) {
      let imageUrl = c.gif_url;

      // JIKA gif_url HANYA BERISI ID (tidak mengandung http/https/blob)
      // Maka kita rangkai menjadi URL Giphy yang valid
      if (!imageUrl.includes("http") && !imageUrl.startsWith("blob:")) {
        imageUrl = `https://media.giphy.com/media/${imageUrl}/giphy.gif`;
      } else if (!imageUrl.startsWith("blob:")) {
        // Jika sudah berupa URL, coba ambil dari cache module gif.js jika ada
        try {
          imageUrl = await gif.get(c.gif_url).catch(() => c.gif_url);
        } catch (e) {
          imageUrl = c.gif_url;
        }
      }
      ...
      return (
        ...
        `
            <div class="d-flex justify-content-center align-items-center my-2">
                <img src="${imageUrl}" id="img-gif-${c.uuid}" class="img-fluid mx-auto gif-image rounded-4" alt="selected-gif" onerror="this.src='https://media.giphy.com/media/${c.gif_url}/giphy.gif'">
            </div>`
      );
    }
