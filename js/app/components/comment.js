...
export const comment = (() => {
  ...
  const update = async (button) => {
    ...
    if (gifIsOpen && gifId) {
      // Ambil URL preview dari kotak pencarian gif
      const previewUrl = document
        .getElementById(`gif-result-${id}`)
        ?.querySelector("img")?.src;

      const imgElement = document.getElementById(`img-gif-${id}`);
      if (imgElement && previewUrl) {
        imgElement.src = previewUrl; // Update gambar di UI
      } else if (!imgElement && previewUrl) {
        // Kasus jika sebelumnya teks lalu diubah jadi GIF,
        // kamu mungkin perlu merender ulang body content-nya.
        location.reload();
      }
  ...
  const send = async (button) => {
    ...
    // --- BAGIAN GIF ---
    const gifContextId = id ? id : gif.default;
    const gifIsOpen = gif.isOpen(gifContextId);
    const gifId = gif.getResultId(gifContextId);
    const gifCancel = gif.buttonCancel(id);

    // Ambil URL preview GIF sebelum form direset/dikirim
    let selectedGifUrl = null;
    if (gifIsOpen && gifId) {
      selectedGifUrl = document
        .getElementById(`gif-result-${gifContextId}`)
        ?.querySelector("img")?.src;
      gifCancel.hide();
    }
    ...
    // SUNTIKKAN URL GIF KE DATA RESPONSE AGAR CARD.JS BISA MERENDER GAMBAR
    if (gifIsOpen && gifId && selectedGifUrl) {
      response.data.gif_url = selectedGifUrl;
    }
